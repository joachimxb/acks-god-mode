...
// self-registers its garrison-units-to-units load-migration (order 150). The five
// privates it exports (_resolveDomain / _levyMusterNoun / _completeTraining /
// _favorDutyMaterializeTroops / _favorDutyDematerializeTroops) are reached here via ACKS.*.
// =============================================================================
...
function commitLevyMusterRecord(campaign, record){
  ...
  if((u.musterPending || 0) <= 0 || ms.arrivedSoFar >= ms.total){
    ...
    u.history.push({ turn, type: 'mustered', text: 'Muster complete — ' + ACKS.unitActiveCount(u) + ' ' + ACKS._levyMusterNoun(u.source) + ' assembled' });
  }
}
