...
### Changed
- **Day-tick review attributes pause details to each item.** Instead of repeating every pause reason in a banner at the top, the review panel now shows a one-line “paused” note and lists each consumer's notable events (lost / hungry / dehydrated / encounter) in red **under its own record** — so with several journeys in flight, each set of warnings sits beneath the journey it belongs to, without re-printing the journey's route.
