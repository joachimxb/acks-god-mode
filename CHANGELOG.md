...
- **Pace can be changed mid-journey.** Pace was previously fixed when the journey started; the Journey Detail panel now has a **Current pace** control (Cautious ×½ / Normal / Forced march ×1½) you can change at any point while travelling. The next day ticked uses the new pace, and each day's record keeps the pace it was actually travelled at (so the day log shows where the party sped up or slowed down). The change is logged like any other edit (e.g. “Set ‹journey› to forced march pace (was normal)”).
