...
### Changed
- The per-domain **Hexes** table (Domain ▸ Demographics) no longer has a row-level **×** delete button. To take a hex out of a domain, open it and set its **Domain** to *Unclaimed wilderness* (which keeps the data consistent, unlike the old button).
