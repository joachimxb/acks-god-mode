...
- **A domain's "+ add hex" now places the hex on the map.** Instead of dropping a blank hex at a placeholder coordinate, the per-domain Hexes table's "+ add hex" (Domain ▸ Demographics) opens the Map in Add/Edit mode with this domain pre-selected in the usual hex picker — click an empty cell to place the new hex (it's claimed for the domain) and you're returned to the domain view; cancelling brings you straight back too.
