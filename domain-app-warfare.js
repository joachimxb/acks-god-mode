...
(function(){
  ...
  unitSheetDomain(){ const u = this.unitSheetUnit(); if(!u) return null; const id = u.homeDomainId || ((u.stationedAt && u.stationedAt.kind === 'domain-garrison') ? u.stationedAt.id : null); return id ? (this.domains || []).find(d => d && d.id === id) : null; },
  unitSheetUnit(){
  ...
  unitSheetHexOptions(){ const u = this.unitSheetUnit(); return u ? this.unitHomeHexOptions(u) : []; },
  ...
  // The unit's DEPLOYMENT — the read-only column (Domains ▸ Military). When NOT in a field army it
  // reads its garrison post (its home hex within the domain, else the domain seat); when IN an army
  // it names the army AND where the army is; rallying reads the muster it marches to; a militia sent
  // home reads "at home". (2026-06-16: was unitLocationLabel; the army branch gained the location.)
  unitDeploymentLabel(unit){
    const camp = this.currentCampaign; if(!camp || !unit) return '—';
    if(unit.rallyingToArmyId){
      const a = (camp.armies || []).find(x => x && x.id === unit.rallyingToArmyId);
      return '🚶 to ' + (a ? (a.name || 'muster') : 'muster');
    }
    if(unit.marchJourneyId){   // a free march (Garrison ▸ March) — where it's headed
      const j = (camp.journeys || []).find(x => x && x.id === unit.marchJourneyId);
      if(j && j.status === 'in-transit') return '🚶 marching to ' + this.hexLabelById(j.destinationHexId);
    }
    if(unit.musterState && (unit.musterPending || 0) > 0){   // W7 — levied troops arriving over 3 weeks (RR p.430)
      const A = window.ACKS;
      const left = (A && A.unitMusterDaysLeft) ? A.unitMusterDaysLeft(camp, unit) : null;
      const total = unit.musterState.total || 0;
      const here = Math.max(0, (unit.count || 0) - (unit.casualties || 0));
      return '⏳ mustering — ' + here + ' of ' + total + ' arrived' + (left != null ? ' (' + left + 'd left)' : '');
    }
    if(unit.trainingState && unit.trainingState.targetTroopType){   // W7 — drilling until the type's months elapse (RR p.431)
      const A = window.ACKS;
    ...
    if(unit.source === 'militia' && unit.calledUp === false){
      return '🏠 at home' + (unit.homeHexId ? (' (' + this.hexLabelById(unit.homeHexId) + ')') : '');
    }
    ...
    const hexId = (st && st.kind === 'domain-garrison' && unit.homeHexId)
      ? unit.homeHexId
      : (window.ACKS.unitCurrentHexId ? window.ACKS.unitCurrentHexId(camp, unit) : null);
  ...
  unitIsDeployed(unit){ return !!(unit && (unit.rallyingToArmyId || (unit.stationedAt && unit.stationedAt.kind === 'army'))); },
  ...
  unitIsMustering(unit){ return !!(unit && (unit.musterPending || 0) > 0); },
  ...
  unitMusterText(unit){
    ...
    const total = (unit.musterState && unit.musterState.total) || 0;
    const here = Math.max(0, (unit.count || 0) - (unit.casualties || 0));
  ...
  // ── March a garrison unit (the Garrison-table "March" verb, 2026-06-17) ──
  // A lone unit is "marching" when it carries a live free-march journey (startUnitMarch). While
  // marching it stays in the Garrison table (its home is still recorded) reading "🚶 marching to …".
  unitIsMarching(u){
    const camp = this.currentCampaign;
    if(!camp || !u || !u.marchJourneyId) return false;
    const j = (camp.journeys || []).find(x => x && x.id === u.marchJourneyId);
    return !!(j && j.status === 'in-transit');
  },
  armyMarchGo(){
  ...
  unitStationLabel(u){
    if(!u) return '—';
    if(u.returnJourneyId){ const d = window.ACKS.unitHomeDomainId ? (this.currentCampaign?.domains||[]).find(x => x && x.id === window.ACKS.unitHomeDomainId(this.currentCampaign, u)) : null; return '🚶 marching home' + (d ? (' to ' + (d.name || d.id)) : ''); }
    if(u.rallyingToArmyId){ const a = window.ACKS.findArmy(this.currentCampaign, u.rallyingToArmyId); return '🚶 marching to ' + (a ? (a.name || 'an army') : 'an army'); }
    const st = u.stationedAt;
    if(!st) return 'unassigned';
    if(st.kind === 'domain-garrison'){ const d = (this.currentCampaign?.domains||[]).find(x => x && x.id === st.id); return 'garrison · ' + (d ? (d.name || d.id) : st.id); }
  ...
  // The hexes a unit may call home: its own domain's hexes (homeDomainId / current garrison),
  // or — for a domain-less unit — any domain hex (picking one assigns the home domain).
  unitHomeHexOptions(unit){
    const camp = this.currentCampaign; if(!camp || !unit) return [];
    const homeDomId = window.ACKS.unitHomeDomainId ? window.ACKS.unitHomeDomainId(camp, unit) : null;
    const inDomain = (camp.hexes || []).filter(h => h && h.domainId);
    return homeDomId ? inDomain.filter(h => h.domainId === homeDomId) : inDomain;
  },
