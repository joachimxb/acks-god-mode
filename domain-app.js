...
function buildDomainFromTemplate(tpl){
  ...
  d.garrison = { units:(tpl.garrison||[]).map(u => ({
    ...
    count:u.count||0, monthlyWage:u.monthlyWage||0, brPerSoldier:u.brPerSoldier||0, stationedAtHexId:null
  })) };
...
const _component = {
  ...
  openLevyModal(domain, source){ if(domain) this.levyModal = { open: true, domainId: domain.id, source, count: 1, homeHexId: '' }; },
  ...
  submitLevy(){
    ...
    if(this.levyModal.homeHexId) this.setUnitHomeUi(u, this.levyModal.homeHexId);
    this.markDirty(); this.schedulePersist();
    ...
    try {
      const ev = A.newEvent('gm-fiat', {
        ...
        payload: {
          ...
          mutation: { fieldPath: 'stationedAtHexId', newValue: u.stationedAtHexId },
  ...
  splitGarrisonUnit(domain, unit){
    ...
    const newUnit = window.ACKS.blankUnit({
      ...
      stationedAtHexId: unit.stationedAtHexId
    });
    unit.count = cur - n;
  ...
  trainModalBudget(){
    const u = this.trainModalUnit(); if(!u || !u.homeDomainId) return null;
    const A = window.ACKS; const src = u.source;
    const d = (this.domains || []).find(x => x && x.id === u.homeDomainId) || null;
    const pool = A.domainLevyPoolCount(this.currentCampaign, u.homeDomainId, src);
    let untrained = 0;
    for(const x of (A.domainLevyUnits(this.currentCampaign, u.homeDomainId, src) || [])){ if(x && x.unitTypeKey === 'untrained-levy') untrained += A.unitActiveCount(x); }
  ...
  trainModalTypeRows(){
    ...
    const pool = A.domainLevyPoolCount(this.currentCampaign, u.homeDomainId, u.source);   // RR p.431 — the cap is pool-wide
    return (A.trainableTroopTypes(race) || []).map(key => {
      ...
      const already = A.domainLevyTrainedOfType(this.currentCampaign, u.homeDomainId, u.source, key);
  ...
  trainModalTreasury(){ const u = this.trainModalUnit(); if(!u || !u.homeDomainId) return 0; const d = (this.domains || []).find(x => x && x.id === u.homeDomainId); return d ? Math.round((d.treasury && d.treasury.gp) || 0) : 0; },
  ...
  domainSplitLevy(domain, unit){
    ...
    nu.homeDomainId = unit.homeDomainId; nu.calledUp = unit.calledUp; nu.moraleAdjustment = unit.moraleAdjustment; nu.loyalty = unit.loyalty;
  ...
  garrisonStandingUnits(domain){
    ...
    if(camp && Array.isArray(camp.units) && A && A.unitHomeDomainId){
      for(const u of camp.units){ if(u && u.source !== 'conscript' && u.source !== 'militia' && A.unitHomeDomainId(camp, u) === domain.id) byId.set(u.id, u); }
    }
  ...
  garrisonDeployUnits(){
    ...
    return ((camp.units) || []).filter(u => u && Math.max(0, (u.count || 0) - (u.casualties || 0)) > 0
        && !(u.stationedAt && u.stationedAt.kind === 'army') && !u.rallyingToArmyId)
      .map(u => {
  ...
  musterAvailableUnits(){
    const camp = this.currentCampaign; if(!camp) return [];
    return ((camp.units) || [])
      .filter(u => u && Math.max(0,(u.count||0)-(u.casualties||0)) > 0 && !(u.stationedAt && u.stationedAt.kind === 'army') && !u.rallyingToArmyId)
  ...
  musterUnitSourceTag(u){
    ...
    if(u.source === 'militia') return ' · 🛡 militia' + (u.unitTypeKey === 'untrained-levy' ? ' (untrained)' : '') + (u.calledUp === false ? ' · at home' : '');
    return '';
  },
  musterArmySubmit(){
  ...
  // a unit home-stationed here (unitHomeDomainId), the ruler commanding in person, or a reaction
  ...
  domainFieldArmyRows(domain){
    ...
    const belongs = (a) => {
      ...
      return units.some(u => u && A.unitHomeDomainId && A.unitHomeDomainId(camp, u) === domain.id);
    };
  ...
  // "🚶 March": open the March modal seeded with the unit + its current location. Like Start-a-Journey
  // at unit scale (destination + 🗺 map-pick + pace + distance preview), but no participants + no supply.
  openMarchModal(u){
    const A = window.ACKS, camp = this.currentCampaign;
    if(!A || !camp || !u) return;
    const origin = A.unitCurrentHexId ? A.unitCurrentHexId(camp, u) : null;
    if(!origin){ this.showToast && this.showToast('This unit has no location to march from (set its home hex or station it).', 4000); return; }
    this.marchModal = { open: true, unitId: u.id, originHexId: origin, destHexId: '', pace: 'normal' };
  },
  marchModalUnit(){
  ...
  // Distance preview for the modal (mirrors journeyWizardMiles): mi / hexes / ≈ days at Normal pace.
  marchModalMiles(){
    const m = this.marchModal;
    if(!m || !m.originHexId || !m.destHexId || m.originHexId === m.destHexId) return null;
    try {
      const tj = { startHexId: m.originHexId, destinationHexId: m.destHexId, days: [] };
  ...
  marchPickDest(){
    ...
    this.mapBeginSelect(hexId => { this.marchModal.destHexId = hexId || ''; this.marchModal.open = true; }, 'Click the destination hex for the march, then ✓ confirm.');
  },
  submitUnitMarch(){
    const A = window.ACKS, m = this.marchModal;
    if(!A || !A.startUnitMarch || !m || !m.unitId || !m.destHexId) return;
    const u = this.marchModalUnit();
    const r = A.startUnitMarch(this.currentCampaign, m.unitId, { destinationHexId: m.destHexId, pace: m.pace });
    if(r && r.ok){
      this.marchModal = { open: false, unitId: null, originHexId: null, destHexId: '', pace: 'normal' };
      this.markDirty(); this.schedulePersist();
      this.showToast && this.showToast('🚶 ' + ((u && u.displayName) || 'Unit') + ' sets out — the Day Clock marches it (+1 day).', 3500);
    } else {
      const why = { 'in-army': 'it is in a field army (it moves with the army)', 'already-marching': 'it is already on the march', 'no-destination': 'pick a destination', 'no-position': 'the unit has no location', 'already-there': 'it is already at that hex' }[r && r.reason] || (r && r.reason) || 'unknown';
      this.showToast && this.showToast('Cannot march — ' + why + '.', 3500);
    }
  },
  stopUnitMarchUi(u){
    const A = window.ACKS; if(!A || !A.stopUnitMarch) return;
    const r = A.stopUnitMarch(this.currentCampaign, u);
    if(r && r.ok){ this.markDirty(); this.schedulePersist(); this.showToast && this.showToast('🛑 ' + ((u && u.displayName) || 'Unit') + ' halts' + (r.haltHexId ? ' at ' + this.hexLabelById(r.haltHexId) : '') + '.', 3500); }
    else { this.showToast && this.showToast('Could not stop the march' + (r && r.reason ? ' (' + r.reason + ')' : '') + '.', 3000); }
  },
  ...
  // ── Unit home garrison (2026-06-14) — the hex a unit defaults to + returns to on task-end ──
  // The hexes of a given domain (the garrison-table picker — home must be a hex in THIS domain).
  domainHomeHexOptions(domain){
    if(!domain || !this.currentCampaign) return [];
    return (this.currentCampaign.hexes || []).filter(h => h && h.domainId === domain.id);
  },
  // Set/clear a unit's home garrison (routes through the engine setter — validates the hex is
  // inside a domain, syncs homeDomainId, stamps unit.history). Mirrors the W7 levy handlers.
  setUnitHomeUi(unit, hexId){
    const r = window.ACKS.setUnitHome(this.currentCampaign, unit, hexId || null);
    if(!r || !r.ok){ if(this.showToast) this.showToast('Could not set home: ' + ((r && r.reason) || 'unknown'), 4000); return; }
    this.markDirty(); this.schedulePersist();
    if(this.showToast) this.showToast(hexId ? 'Home garrison set.' : 'Home garrison cleared.');
  },
  ...
  levyModal: { open: false, domainId: null, source: 'conscript', count: 1, homeHexId: '' },  // ⚒/🛡 Levy modal
  ...
  marchModal: { open: false, unitId: null, originHexId: null, destHexId: '', pace: 'normal' },  // 🚶 March a garrison unit
